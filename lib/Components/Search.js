...
export default class Search extends React.Component {
  ...
  getLocation(place) {
    ...
    return () => (this.setState({ search: '', suggestedCities: []}))
  }
    ...
    return (
      <div className={this.props.searchDiv}>
        ...
        <ul className = 'suggestions-list'>
            {suggestedCities.map( (city, i) => <li key={i} onClick = {(e) => this.setState({ search: e.target.innerHTML, suggestedCities: []})}>{city}</li>)}
        </ul>
